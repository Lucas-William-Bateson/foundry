...
pub fn start_foundryd_watchdog() {
    tokio::spawn(async move {
        info!("🐕 Starting foundryd watchdog");
        ...
        loop {
            ...
            match check_container_health("foundry-foundryd-1").await {
                Ok(true) => {
                    if consecutive_failures > 0 {
                        info!("🐕 foundryd container recovered");
                    }
                ...
                Ok(false) => {
                    ...
                    warn!(
                        "🐕 foundryd container unhealthy ({}/{})",
                        consecutive_failures, UNHEALTHY_THRESHOLD
                    );
                    
                    if consecutive_failures >= UNHEALTHY_THRESHOLD {
                        error!("🐕 foundryd container appears down, attempting restart...");
                        if let Err(e) = restart_container("foundry-foundryd-1").await {
                            error!("🐕 Failed to restart foundryd: {}", e);
                        } else {
                            info!("🐕 foundryd container restart initiated");
                            consecutive_failures = 0;
                ...
                Err(e) => {
                    warn!("🐕 Failed to check foundryd health: {}", e);
                }
            }
        }
    });
