...
use crate::infrastructure::docker_runner::build_image;
...
pub(crate) async fn run_deploy(
    ...
    logger.log(job, &format!("🚀 Deploying {}", app_name)).await?;

    if let Some(compose_file) = &untrusted_repo_config.deploy.compose_file {
        logger.log(job, &format!("Using compose file: {}", compose_file)).await?;
        ...
        if !untrusted_repo_config.env.is_empty() {
            logger.log(job, &format!("🔐 Injecting {} env var(s) from Vault", untrusted_repo_config.env.len())).await?;
        }
        ...
        logger.log(job, &format!("Stopping existing {} containers...", app_name)).await?;
        ...
        if !output.status.success() {
            ...
            logger.log(job, &format!("Deploy failed: {}", stderr)).await?;
        ...
        logger.log(job, &format!("Stopping existing container: {}", container_name)).await?;
        let _ = Command::new("docker")
        ...
        logger.log(job, &format!("Starting container: {}", container_name)).await?;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            logger.log(job, &format!("Failed to start: {}", stderr)).await?;
    ...
    if !domains.is_empty() {
        ...
        logger.log(job, &format!("🌐 Configuring {} domain route(s) -> port {}", domains.len(), port)).await?;
        
        for domain in domains {
            match setup_domain_route(domain, port).await {
                Ok(()) => {
                    logger.log(job, &format!("✅ Domain configured: https://{}", domain)).await?;
                }
                Err(e) => {
                    logger.log(job, &format!("⚠️ Failed to setup domain route for {}: {}", domain, e)).await?;
    ...
    logger.log(job, &format!("✅ {} deployed successfully", app_name)).await?;
    Ok(())
}
