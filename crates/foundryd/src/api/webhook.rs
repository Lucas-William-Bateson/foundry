...
use crate::{infrastructure::db::{self, PushEventData, PullRequestEventData, RepoData}, AppState};

pub fn router() -> Router<Arc<AppState>> {
    Router::new().route("/webhook/github", post(receive_webhook))
}
...
async fn receive_webhook(
    ...
    match event_type {
        "push" => enqueue_jobs_for_push(&state, &body).await,
        "pull_request" => enqueue_jobs_for_pull_request(&state, &body).await,
        _ => {
...
async fn enqueue_jobs_for_push(
    state: &Arc<AppState>,
    body: &Bytes,
...
async fn enqueue_jobs_for_pull_request(
    state: &Arc<AppState>,
    body: &Bytes,
) -> (StatusCode, Json<ApiResponse>) {
